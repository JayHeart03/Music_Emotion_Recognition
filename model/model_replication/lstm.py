...
data_pad = pad_sequences(sequences, maxlen=1000)
...
model = Sequential([
    Embedding(input_dim=vocab_size, output_dim=embedding_dim, weights=[embedding_matrix], input_length=1000, trainable=False),  # 使用GloVe嵌入
    Dropout(0.2), 
    LSTM(100),
    Dense(4, activation='softmax',activity_regularizer=l2(0.001))  # 密集层，4个输出单元，用于分类
])
