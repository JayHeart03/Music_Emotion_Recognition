...
# Function to validate the song information
...
def extract_lyrics_and_save_to_csv(input_file, output_file):
    ...
    for index, row in df.iterrows():
        if pd.isna(row['Lyrics']) or row['Lyrics'] == '':
            lyrics = Extract(row['track_name'], row['artist'])
...
def main():
    for year in range(2013, 2024): 
        ...
        output_file_path = f'data/data_top100/data_top100_raw/{year}_processed_lyrcis.csv'
        extract_lyrics_and_save_to_csv(input_file_path, output_file_path)
        print(f'Processed lyrics for year {year}')
